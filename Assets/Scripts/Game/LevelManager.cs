...
public class LevelManager : MonoBehaviourPunCallbacks
{
   ...
   [PunRPC]
    ...
    private void CarInstance()
    {
        if (SceneMediator.IsPhoton)
        {
            _Car = PhotonNetwork.Instantiate(SceneMediator.Car.name, _Start_Position.position, Quaternion.identity).GetComponent<Car>();
            return;
        }
        
        _Car = Instantiate(SceneMediator.Car, _Start_Position.position, Quaternion.identity);
    }
